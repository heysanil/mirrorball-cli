  ...
  .github/workflows/release.yml
        ...
        └─ npm job     bun scripts/build-npm-packages.ts --source release
                       → npm/mirb-<target>/  ×5   (os/cpu-gated, one binary each)
                         npm/mirb-cli/                (the shim + optionalDependencies)
...
- **`mirb-cli-darwin-arm64`, `mirb-cli-darwin-x64`, `mirb-cli-linux-arm64`, `mirb-cli-linux-x64`,
  `mirb-cli-windows-x64`** — one binary each, gated by npm's `os` and `cpu` fields so the four
